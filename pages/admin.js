...
export default function Admin() {
  ...
  function handleLogin(e) {
    e.preventDefault()
    if (password === process.env.NEXT_PUBLIC_ADMIN_PASSWORD || password === 'victoria2024') {
      setAuth(true)
    } else {
  ...
  async function createPost(e) {
    e.preventDefault()
    if (!newCaption || uploadFiles.length === 0) {
      alert('Ajoute une caption et au moins une photo')
      return
    }
    setUploading(true)
    ...
    const { data: post, error } = await supabase
      .from('posts')
      .insert({ caption: newCaption, price: newPrice })
      .select()
      .single()
    ...
    for (let i = 0; i < uploadFiles.length; i++) {
      ...
      if (uploadError) { alert('Erreur upload photo ' + i); continue }
        ...
        <div style={{ minHeight: '100vh', background: '#080808', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'system-ui' }}>
          <form onSubmit={handleLogin} style={{ background: '#141414', padding: '40px', borderRadius: '16px', width: '320px' }}>
            ...
            <input
              ...
              style={{ width: '100%', padding: '12px 16px', background: '#222', border: '1px solid #333', borderRadius: '10px', color: '#fff', fontSize: '14px', marginBottom: '16px', outline: 'none' }}
            />
            <button type="submit" style={{ width: '100%', padding: '12px', background: '#fff', color: '#080808', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: '500', cursor: 'pointer' }}>
              Connexion
            </button>
          </form>
        </div>
      ...
      <div style={{ minHeight: '100vh', background: '#080808', color: '#fff', fontFamily: 'system-ui', maxWidth: '600px', margin: '0 auto', padding: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
          {['posts', 'nouveau'].map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ padding: '8px 18px', background: tab === t ? '#fff' : 'transparent', color: tab === t ? '#080808' : 'rgba(255,255,255,0.5)', border: '1px solid ' + (tab === t ? '#fff' : 'rgba(255,255,255,0.15)'), borderRadius: '50px', fontSize: '13px', cursor: 'pointer', textTransform: 'capitalize' }}>
              {t === 'nouveau' ? '+ Nouveau post' : 'Mes posts'}
            </button>
          ))}
        </div>
        ...
        {tab === 'nouveau' && (
          <form onSubmit={createPost} style={{ background: '#141414', borderRadius: '16px', padding: '20px' }}>
            ...
            <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Caption</label>
            <input
              value={newCaption}
              onChange={e => setNewCaption(e.target.value)}
              placeholder="Description du post..."
              style={{ width: '100%', padding: '12px', background: '#1e1e1e', border: '1px solid #2a2a2a', borderRadius: '10px', color: '#fff', fontSize: '14px', marginBottom: '16px', outline: 'none' }}
            />

            <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>Prix pour débloquer (€)</label>
            <input
              type="number"
              value={newPrice}
            ...
            <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', letterSpacing: '0.08em', textTransform: 'uppercase', display: 'block', marginBottom: '8px' }}>
              Photos (la 1ère est gratuite, les suivantes sont floutées)
            </label>
            <input
              type="file"
              multiple
              ...
              style={{ width: '100%', padding: '12px', background: '#1e1e1e', border: '1px dashed #2a2a2a', borderRadius: '10px', color: 'rgba(255,255,255,0.5)', fontSize: '13px', marginBottom: '8px', cursor: 'pointer' }}
            />
            {uploadFiles.length > 0 && (
              <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginBottom: '16px' }}>
                {uploadFiles.length} photo(s) sélectionnée(s) — photo 1 visible, photos 2-{uploadFiles.length} floutées
              </p>
            )}
        ...
        {tab === 'posts' && (
          <div>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '40px', color: 'rgba(255,255,255,0.3)' }}>Aucun post. Crée ton premier post !</div>
            ) : (
              posts.map(post => (
                <div key={post.id} style={{ background: '#141414', borderRadius: '16px', padding: '16px', marginBottom: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    ...
                    <button onClick={() => deletePost(post.id)} style={{ background: 'rgba(255,50,50,0.15)', color: '#ff6b6b', border: 'none', borderRadius: '8px', padding: '6px 12px', fontSize: '12px', cursor: 'pointer' }}>
                      Supprimer
                    </button>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    {(post.photos || []).map((photo, i) => (
                      <div key={photo.id} style={{ position: 'relative', width: '80px', height: '80px', borderRadius: '8px', overflow: 'hidden', border: '2px solid ' + (photo.is_locked ? '#ff6b6b' : '#4caf50') }}>
                        <img src={getPhotoUrl(photo.url)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: photo.is_locked ? 'blur(4px)' : 'none' }} />
                        <div style={{ position: 'absolute', bottom: '2px', left: '0', right: '0', display: 'flex', justifyContent: 'center', gap: '4px' }}>
                          <button
                            onClick={() => toggleLock(photo.id, photo.is_locked)}
                            title={photo.is_locked ? 'Déflouter' : 'Flouter'}
                            style={{ background: 'rgba(0,0,0,0.7)', color: '#fff', border: 'none', borderRadius: '4px', padding: '2px 4px', fontSize: '10px', cursor: 'pointer' }}
                          >
                            {photo.is_locked ? '🔒' : '🔓'}
                          </button>
                          <button
                            onClick={() => deletePhoto(photo.id, photo.url)}
                            style={{ background: 'rgba(200,0,0,0.7)', color: '#fff', border: 'none', borderRadius: '4px', padding: '2px 4px', fontSize: '10px', cursor: 'pointer' }}
                          >
                            ✕
                          </button>
                        </div>
                        <div style={{ position: 'absolute', top: '2px', left: '4px', fontSize: '9px', color: '#fff', background: 'rgba(0,0,0,0.6)', borderRadius: '3px', padding: '1px 3px' }}>
                          {i + 1}
                        </div>
                      </div>
                    ))}
