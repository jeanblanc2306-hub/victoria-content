...
function useCountdown() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setSeconds(function(s) { return s <= 1 ? 900 : s - 1 })
    }, 1000)
...
function PromoBanner() {
  ...
  if (minimized) {
    return (
      <div onClick={function() { setMinimized(false) }} style={{ position: "fixed", bottom: "20px", left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg, #ff4e00, #ec9f05)", borderRadius: "30px", padding: "8px 16px", display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", zIndex: 999, boxShadow: "0 4px 20px rgba(255,78,0,0.4)" }}>
  ...
  return (
    <div style={{ position: "fixed", bottom: "20px", left: "50%", transform: "translateX(-50%)", width: "calc(100% - 32px)", maxWidth: "440px", background: "linear-gradient(135deg, #ff4e00, #ec9f05)", borderRadius: "16px", padding: "14px 16px", zIndex: 999, boxShadow: "0 4px 24px rgba(255,78,0,0.45)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={function() { setMinimized(true) }} style={{ background: "rgba(255,255,255,0.25)", border: "none", borderRadius: "50%", width: "26px", height: "26px", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff", fontSize: "14px", flexShrink: 0 }}>X</button>
      </div>
    </div>
  )
}
...
function Carousel({ photos, idx, isUnlocked, getUrl, onSlide, onUnlock, price }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", aspectRatio: "4/5", background: "#111", overflow: "hidden", userSelect: "none", touchAction: "pan-y" }} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
      {photos.map(function(photo, i) {
        ...
        return (
          <div key={photo.id} style={{ position: "absolute", inset: 0, transition: "transform 0.3s ease", transform: "translateX(" + ((i - idx) * 100) + "%)" }}>
            <img src={getUrl(photo.url)} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", filter: shouldBlur ? "blur(14px)" : "none", transform: shouldBlur ? "scale(1.05)" : "none" }} />
            {shouldBlur && (
      ...
      {photos.length > 1 && (
        <>
          {idx > 0 && (
            <button onClick={function() { onSlide(idx - 1) }} style={{ position: "absolute", left: "10px", top: "50%", transform: "translateY(-50%)", width: "32px", height: "32px", borderRadius: "50%", background: "rgba(255,255,255,0.85)", border: "none", cursor: "pointer", fontSize: "20px", display: "flex", alignItems: "center", justifyContent: "center", color: "#080808", zIndex: 5 }}>&#8249;</button>
          )}
          {idx < photos.length - 1 && (
            <button onClick={function() { onSlide(idx + 1) }} style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", width: "32px", height: "32px", borderRadius: "50%", background: "rgba(255,255,255,0.85)", border: "none", cursor: "pointer", fontSize: "20px", display: "flex", alignItems: "center", justifyContent: "center", color: "#080808", zIndex: 5 }}>&#8250;</button>
          )}
          <div style={{ position: "absolute", bottom: "10px", left: "50%", transform: "translateX(-50%)", display: "flex", gap: "4px", zIndex: 5 }}>
            {photos.map(function(_, i) {
              return <div key={i} style={{ width: "5px", height: "5px", borderRadius: "50%", background: i === idx ? "#fff" : "rgba(255,255,255,0.3)", transition: "background 0.2s" }} />
            })}
          </div>
        </>
      )}
  ...
  async function fetchPosts() {
    ...
    if (data) {
      setPosts(data.map(function(p) {
        return { ...p, photos: (p.photos || []).sort(function(a, b) { return a.position - b.position }) }
      }))
    }
    setLoading(false)
  }
  ...
  useEffect(function() {
    ...
    if (params.get("success") === "1") {
      ...
      if (postId) {
        const saved = JSON.parse(localStorage.getItem("unlocked_posts") || "{}")
        saved[postId] = true
        localStorage.setItem("unlocked_posts", JSON.stringify(saved))
        setUnlocked({ ...saved })
      }
              ...
              return (
                <div key={post.id} className="post">
                  ...
                  <Carousel
                    ...
                    getUrl={getUrl}
                    onSlide={function(next) { setCarouselIndex(function(prev) { return { ...prev, [post.id]: next } }) }}
                    onUnlock={function() { setModal(post) }}
                    price={post.price}
                  />
        ...
        {modal && (
          <div className="modal-bg" onClick={function() { setModal(null) }}>
            <div className="modal" onClick={function(e) { e.stopPropagation() }}>
              ...
              <button className="btn-cancel" onClick={function() { setModal(null) }}>Annuler</button>
            </div>
          </div>
        )}
